...
def process_file(ufilename):
    ...
    for line in ulines:
        ...
        try:
            ...
            print("{} not formatted correctly in line {}".format(ufilename,ulines.index(line)))
            exit(1)
    ...
    # calculate stats
            ...
            for k in range(1,kmax): #sum for each spectra
                ...
                if E_k < 0 and flag == False:
                    
                    print('Warning:{} Negative Energies in line {:d}'.format(ufilename,i))
                    flag = True
...
if directory == '':
    ...
    Path = os.path.join(directory,'*.spc')
    spc_files = glob(Path)

spc_files.sort()                        # Sorts files by numerical/alph order
